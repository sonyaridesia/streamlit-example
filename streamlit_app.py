...
def main():
    ...
    if uploaded_file is not None:
        if st.button("Process CSV"):
            ...
            # Menambahkan fitur one-hot encoding
            st.write("## One-Hot Encoding:")
            categorical_columns = df.select_dtypes(include='object').columns
            selected_columns = st.multiselect("Select columns to one-hot encode", categorical_columns)

            if selected_columns:
                one_hot_df = pd.get_dummies(df, columns=selected_columns)
